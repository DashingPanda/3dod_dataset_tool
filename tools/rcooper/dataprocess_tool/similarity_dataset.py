...
from lib.data_utils import md5
from lib.utils import common_utils

def args_parser():
    parser = argparse.ArgumentParser(description="synthetic data generation")
    parser.add_argument('--dataset1_txt', type=str, required=True,
                        help='The path of the txt file of the first dataset')
    parser.add_argument('--dataset2_txt', type=str, required=True,
                        help='The path of the txt file of the second dataset')
...
def retrieve_overlap_rope3d_images(dataset1_txt, dataset2_txt, log_file):
    dir_path = '/mnt/data_cfl/Projects/Data/Rope3D_data/image_2'
    ...
    # f = open(log_file, 'w')
    for image_name in tqdm(dataset1_image_names):
        # if index == 10:
        #     break
        ...
        index += 1
    for image_name in tqdm(dataset2_image_names):
        # if index == 10:
        #     break
        img = cv2.imread(image_name)
        gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ...
    for img_name_dataset2 in tqdm(grayimg_dataset2_dict): 
...
def main():
    opt = args_parser()
    dataset1_txt = opt.dataset1_txt
    dataset2_txt = opt.dataset2_txt
    log_file = opt.log_file
    retrieve_overlap_rope3d_images(dataset1_txt, dataset2_txt, log_file)
