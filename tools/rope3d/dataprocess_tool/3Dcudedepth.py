...
def get_3dbox_corners_img_cam(objs: list, cam_intri: np.ndarray, cam2world_extri: np.ndarray, world2cam_extri: np.ndarray) -> np.ndarray:
    ...
    obj_location_dimensions_rotation_cam = common_utils.get_location_dimensions_rotation(objs)
...
def mask_3dbox_img(o_corners_img : np.ndarray):
    ...
    # create a image with zeros of size 1920x1080x3
    cude_mask_obj = np.zeros((1080, 1920, 3), dtype=np.uint16)
...
def mask_3dboxplaen_img(p_corners_img):
    ...
    # create a image with zeros of size 1920x1080x3
    plane_mask_obj = np.zeros((1080, 1920, 3), dtype=np.uint16)
    # convert the o_corners_img to int
...
def generate_cude_depth_per_plane(p_corners_img: np.ndarray, coefficient_plane: np.ndarray, fx: float, fy: float, cx: float, cy: float):
    ...
    Returns
    -------
    plane_depth_img : np.ndarray, shape (1080, 1920, 3)
    ...
    plane_depth_img = np.zeros((1080, 1920, 3), dtype=np.uint16)
    plane_mask_obj_r = plane_mask_obj[:,:,0]
    ...
    for v in range(min_y, max_y):
        for u in range(min_x, max_x):
            ...
            if pixel == 255:
                # print(f'u: {u}, v: {v}')
                plane_depth = calculate_the_depth_plane(coefficient_plane, u, v, fx, fy, cx, cy)
...
def combine_into_cude_depth_per_image(cude_depth_objs: np.ndarray, cude_depth_img: np.ndarray):
    ...
    parameters
    ----------
    cude_depth_objs : np.ndarray, shape (N, 1080, 1920, 3), N is the number of objects
        The depths of 6 planes of each 3d bounding box in a image
    """
    h, w, c = cude_depth_objs[0].shape
    for v in range(h):
        for u in range(w):
            # the minimum value of the pixel in plane_depth_imgs and not less than 0
            pixels = cude_depth_objs[:, v, u, 0]
...
def combine_into_cude_depth_per_object(obj: Data, plane_depth_imgs: np.ndarray, cude_depth_img: np.ndarray):
    ...
    plane_depth_imgs : np.ndarray, shape (N, 1080, 1920, 3)
        The depths of 6 planes of a 3d bounding box

    cude_depth_img : np.ndarray, shape (1080, 1920, 3)
    ...
    #             else:
    #                 cude_depth_img[v, u, :] = np.minimum(cude_depth_img[v, u, :], pixel)
    h, w, c = plane_depth_imgs[0].shape
    x1, y1, x2, y2 = obj.x1, obj.y1, obj.x2, obj.y2
    for v in range(y1, y2+1):
        for u in range(x1, x2+1):
            # the minimum value of the pixel in plane_depth_imgs and not less than 0
            pixels = plane_depth_imgs[:, v, u, 0]
            pixel = np.min(pixels[pixels > 0], axis=0) if len(pixels[pixels > 0]) > 0 else 0
            cude_depth_img[v, u, :] = pixel
...
def generate_cude_depth_per_object(obj: Data, o_corners_img: np.ndarray, o_corners_cam: np.ndarray, cam_intri: np.ndarray) -> np.ndarray:
    ...
    cude_depth_img = np.zeros((1080, 1920, 3), dtype=np.uint16) # create a image with zeros of size 1920x1080x3
    plane_depth_imgs = np.zeros((1, 1080, 1920, 3), dtype=np.uint16)
    for (coefficient_plane, corners_plane) in zip(coefficient_6planes, corners_planes):
        ...
        plane_depth_img = plane_depth_img.reshape(1, 1080, 1920, 3)
...
def generate_cude_depth_per_image(objs: list, filename_woe: str, dataset_dir: str):
    ...
    # create a image with zeros of size 1920x1080x3
    cude_depth_img = np.zeros((1080, 1920, 3), dtype=np.uint16)
    cude_depth_objs = np.zeros((1, 1080, 1920, 3), dtype=np.uint16)
    for i, (obj, o_corners_img, o_corners_cam) in enumerate(zip(objs, obj_corners_img, obj_corners_cam)):
        ...
        cude_depth_obj = cude_depth_obj.reshape(1, 1080, 1920, 3)
...
@timer
def generate_cude_depth(dataset_dir):
    """
    """
    label_folder = os.path.join(dataset_dir, 'label_2_4cls_for_train')
    print(f'label_folder: {label_folder}')
    gt_files = common_utils.retrieve_label_files(label_folder, 'rope3d')
    for gt_file in tqdm(gt_files, desc='Processing', colour='green'):
        ...
        gt_file = '/mnt/data_cfl/Projects/Data/Rope3D_data/label_2_4cls_filter_with_roi_for_eval/145048_fa2sd4a08E154AIR_420_1626240189_1626240424_194_obstacle.txt'
        ...
        for gt_result_line in gt_result_lines:
            ...
            obj_data = Data()
            rope3d_labelloader.get_label_data(obj_data, gt_result_line)
            if obj_data.X == 0 and obj_data.Y == 0 and obj_data.Z == 0: # No 3d bounding box
                continue
            if obj_data.occlusion > 1:
                continue
            if obj_data.truncation > 1: # the object is out of image severely
                continue
            objs.append(obj_data)
        cude_depth_img = generate_cude_depth_per_image(objs, filename_woe, dataset_dir)

        output_dir = '/mnt/data_cfl/Projects/3dod-dataset-tools/output'
        img_file = f'{filename_woe}.png'
        common_utils.save_img(cude_depth_img, output_dir, img_file)
...
def main():
    ...
    print('====================================================================================')
    generate_cude_depth(dataset_dir)
